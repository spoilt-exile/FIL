...
(set! fk-clr-stage
  (list
    ...
    (list "Monochrome: b/w"		TRUE	(quote (fil-clr-monochrome fk-sep-image fio_uni_layer fc_imh fc_imw fc_fore 0)))

    ;Process "Monochrome: sepia" with proc_id=3
    (list "Monochrome: sepia"		TRUE	(quote (fil-clr-monochrome fk-sep-image fio_uni_layer fc_imh fc_imw fc_fore 1)))
    ...
    (list "Lomo: Soft yellow" 		FALSE	(quote (fil-clr-lomo fk-sep-image fio_uni_layer 1)))

    ;Process "Lomo: Golden autumn" with proc_id=6
    (list "Lomo: Golden autumn"		FALSE	(quote (fil-clr-lomo fk-sep-image fio_uni_layer 2)))
    ...
    (list "Dram: normal"		TRUE	(quote (fil-clr-dram fk-sep-image fio_uni_layer '(93 103 124))))

    ;Process "Dram: user colors" with proc_id=16
    (list "Dram: user colors"		TRUE	(quote (fil-clr-dram fk-sep-image fio_uni_layer fc_fore)))
  )
)
...
(set! fk-grain-stage
  (list
    ...
    (list "(G'MIC) Sulfide; scratches"	TRUE	(quote (fil-grnfx-sulfide fk-sep-image fio_uni_layer fc_imh fc_imw fc_fore 2.5 FALSE TRUE)))

    ;Process "Dram Grain: normal" with proc_id=6
    (list "(G'MIC) Dram Grain: normal"	TRUE	(quote (fil-grnfx-dram fk-sep-image fio_uni_layer fc_imh fc_imw fc_fore 30 TRUE)))

    ;Process "Dram Grain: light" with proc_id=7
    (list "Dram Grain: light"		TRUE	(quote (fil-grnfx-dram fk-sep-image fio_uni_layer fc_imh fc_imw fc_fore 25 FALSE)))
  )
)
...
(define (fil-gal-check)
  (if (defined? 'gal-info)
	...
	(gimp-message
	  (string-append
	  "GAL revision " (number->string gal-revision) " is too old for this script.\n"
	  "Update GAL using URL:\n"
	  fk-gal-url
	  )
	)
	(quit)
	...
	(string-append
	"GAL not found.\n"
	"Further execution impossible, install GAL using URL:\n"
	fk-gal-url
	)
      )
      (quit)
